...
server.post("/auth/register", (req, res) => {
  ...
  const access_token = createToken({ username }, TOKEN_EXPIRES.ACCESS_TOKEN);
...
server.post("/auth/login", (req, res) => {
  ...
  const access_token = createToken({ username }, TOKEN_EXPIRES.ACCESS_TOKEN);
  const refresh_token = createToken({ username }, TOKEN_EXPIRES.REFRESH_TOKEN);
  console.log("Access Token: " + access_token);
  console.log("Refresh Token: " + refresh_token);

  res
    .status(200)
    .json({ access_token, refresh_token, roles: [ROLES.Admin, ROLES.Editor, ROLES.User], user: "Admin" });
});

// Refresh Token
server.post("/auth/refreshToken", (req, res) => {
  console.log("refreshToken endpoint called; request body:");
  console.log(req.body);
  const { refresh_token: token } = req.body;
  if (token === undefined) {
    const status = 401;
    const message = "Incorrect REFRESH_TOKEN";
    res.status(status).json({ status, message });
    return;
  }
  ...
  const username = isToken.username;
